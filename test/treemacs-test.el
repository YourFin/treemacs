  ...
  (ert-deftest add-to-open-dirs-cache::add-single-item ()
    (with-temp-buffer
      (let ((b (insert-text-button "b"))
            ...
            (child  "/home/A/B")
            (treemacs--open-dirs-cache (make-hash-table :test #'equal)))
        (button-put p 'abs-path "/home/A")
        (button-put b 'abs-path "/home/A/B")
        (button-put b 'parent p)
        (treemacs--add-to-open-dirs-cache b)
        (should (= 1 (hash-table-count treemacs--open-dirs-cache)))
        (should (equal (gethash "/home/A" treemacs--open-dirs-cache)
                       '("/home/A/B"))))))
  ...
  (ert-deftest add-to-open-dirs-cache::add-single-collapsed-item ()
    (with-temp-buffer
      (let ((b (insert-text-button "b"))
        ...
        (treemacs--add-to-open-dirs-cache b)
        (should (= 1 (hash-table-count treemacs--open-dirs-cache)))
        (should (equal (gethash "/home/P" treemacs--open-dirs-cache)
                       '("/home/A/B"))))))
  ...
  (ert-deftest add-to-open-dirs-cache::add-two-different-parent-items ()
    (with-temp-buffer
      (let ((p1 (insert-text-button "p1"))
        ...
        (treemacs--add-to-open-dirs-cache b1)
        (treemacs--add-to-open-dirs-cache b2)
        (should (= 2 (hash-table-count treemacs--open-dirs-cache)))
        (should (equal (gethash "/A1" treemacs--open-dirs-cache)
                       '("/A1/B1")))
        (should (equal (gethash "/A2" treemacs--open-dirs-cache)
                       '("/A2/B2"))))))
...
(progn
  (ert-deftest clear-from-open-dirs-cache::clear-item-from-empty-cache ()
    (let ((treemacs--open-dirs-cache (make-hash-table :test #'equal)))
      (treemacs--remove-from-open-dirs-cache "/home/A")
      (should (= 0 (hash-table-count treemacs--open-dirs-cache)))))

  (ert-deftest clear-from-open-dirs-cache::clear-item-not-in-the-cache ()
    (let ((treemacs--open-dirs-cache (make-hash-table :test #'equal)))
      (puthash "/home/A" '("/home/A/B") treemacs--open-dirs-cache)
  ...
  (ert-deftest clear-from-open-dirs-cache::clear-item-from-empty-cache-with-purge ()
    (let ((treemacs--open-dirs-cache (make-hash-table :test #'equal)))
      ...
      (should (hash-table-empty-p treemacs--open-dirs-cache))))
  ...
  (ert-deftest clear-from-open-dirs-cache::clear-item-from-cache-leaving-other-entry ()
    (let ((treemacs--open-dirs-cache (make-hash-table :test #'equal)))
      (puthash "/home/A" '("/home/A/B") treemacs--open-dirs-cache)
      (puthash "/home/C" '("/home/C/D") treemacs--open-dirs-cache)
      (treemacs--remove-from-open-dirs-cache "/home/A/B")
      (should (= 1 (hash-table-count treemacs--open-dirs-cache)))
      (should (equal (gethash "/home/C" treemacs--open-dirs-cache)
                     '("/home/C/D")))))
  ...
  (ert-deftest clear-from-open-dirs-cache::clear-one-of-two-items ()
    (let ((treemacs--open-dirs-cache (make-hash-table :test #'equal)))
      (puthash "/home/A" '("/home/A/B" "/home/A/C") treemacs--open-dirs-cache)
      (treemacs--remove-from-open-dirs-cache "/home/A/B")
      (should (= 1 (hash-table-count treemacs--open-dirs-cache)))
      (should (equal (gethash "/home/A" treemacs--open-dirs-cache)
                     '("/home/A/C")))))
  ...
  (ert-deftest clear-from-open-dirs-cache::clear-item-and-purge-children ()
    (let ((treemacs--open-dirs-cache (make-hash-table :test #'equal)))
      (puthash "/home/A"         '("/home/A/B") treemacs--open-dirs-cache)
      (puthash "/home/A/B"       '("/home/A/B/C1" "/home/A/B/C2") treemacs--open-dirs-cache)
      (puthash "/home/A/B/C1"    '("/home/A/B/C1/D1" "/home/A/B/C1/D2" "/home/A/B/C1/D3") treemacs--open-dirs-cache)
      (puthash "/home/A/B/C2"    '("/home/A/B/C2/D1") treemacs--open-dirs-cache)
      (puthash "/home/A/B/C1/D3" '("/home/A/B/C1/D3/E1") treemacs--open-dirs-cache)
      (puthash "/home/A2"        '("/home/A2/B2") treemacs--open-dirs-cache)
      (treemacs--remove-from-open-dirs-cache "/home/A/B" t)
      (should (= 1 (hash-table-count treemacs--open-dirs-cache)))
      (should (equal (gethash "/home/A2" treemacs--open-dirs-cache)
                     '("/home/A2/B2"))))))
...
(progn
  (ert-deftest update-caches-after-rename::update-runs-correctly ()
    (let ((treemacs--open-dirs-cache (make-hash-table :test #'equal))
          (treemacs--tags-cache (make-hash-table :test #'equal)))
      (puthash "/old/name/A" '("/old/name/A/B1" "/old/name/A/B2") treemacs--open-dirs-cache)
      (puthash "/other/A"  '("/other/A/B1" "/other/A/B2") treemacs--open-dirs-cache)
      ...
      (should (= 2 (hash-table-count treemacs--open-dirs-cache)))
      (should (equal (gethash "/other/A" treemacs--open-dirs-cache)
                     '("/other/A/B1" "/other/A/B2")))
      (should (equal (gethash "/new/name/A" treemacs--open-dirs-cache)
                     '("/new/name/A/B1" "/new/name/A/B2")))
